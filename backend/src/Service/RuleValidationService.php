...
class RuleValidationService
{
    ...
    private function validateBasicRule(array $levels, int $count): void
    {
        ...
        if ($levels !== $expected) {
            ...
            throw new \InvalidArgumentException('Basic rules must have difficulty levels 1-9 (no gaps). ' . implode('. ', $errors));
        }
    }
    ...
    private function validateCourtRule(array $levels, int $count): void
    {
        ...
        if ($levels !== $expected) {
            ...
            throw new \InvalidArgumentException('Court rules must have difficulty levels 1-4 (no gaps). ' . implode('. ', $errors));
        }
    }
    ...
    private function validateLegendaryRule(array $levels, int $count): void
    {
        ...
        $level = $levels[0];
        if ($level !== 1) {
            throw new \InvalidArgumentException("Legendary rules must have difficulty level 1. Found: {$level}");
        }
    }
}
